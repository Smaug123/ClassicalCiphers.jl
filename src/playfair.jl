...
function playfair_key_to_square(key::AbstractString, replacement)
	...
	remaining = collect(filter(x -> (x != replacement[2] && findfirst(i -> i == x, key_sanitised) == 0), 'A':'Z'))
...
function encrypt_playfair(plaintext, key::Array{Char, 2}; stripped=false, combined=('I', 'J'))
	if !stripped
		if findfirst(i -> i == combined[2], key) != 0
    ...
    while i < length(plaintext_sanitised)
    	...
    	l1pos = CartesianIndices((5, 5), findfirst(i -> i == l1, key, l1))
    	l2pos = CartesianIndices((5, 5), findfirst(i -> i == l2, key, l2))
